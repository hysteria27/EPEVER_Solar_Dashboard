...
<body class="bg-gray-50 dark:bg-slate-950 text-gray-800 dark:text-gray-100 min-h-screen flex flex-col">
    <div class="max-w-7xl mx-auto w-full p-4 md:p-6 lg:p-8">
        ...
        <div id="view-settings" class="hidden-view fade-in space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm">
                <div class="flex justify-between items-center mb-6">
                    <div><h2 class="text-xl font-bold">WiFi Configuration</h2><p class="text-gray-500 text-sm">Connect to WiFi network or manage connectivity.</p></div>
                    <button onclick="fetchWiFiStatus()" class="text-sm text-solar-500 font-bold flex gap-1 items-center" title="Refresh WiFi status"><i data-lucide="refresh-cw" class="w-4 h-4"></i> Refresh</button>
                </div>
                ...
                <div id="wifi-status-container" class="mb-6 p-4 bg-gray-50 dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700">
                    <div class="flex items-center gap-2 mb-2">
                        <span id="wifi-status-dot" class="inline-block w-3 h-3 rounded-full bg-gray-300"></span>
                        <span id="wifi-status-text" class="font-semibold">Checking WiFi status...</span>
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="text-xs font-semibold block mb-1">Network SSID</label>
                            <input type="text" id="wifi-ssid" placeholder="Enter network name" class="w-full p-2 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded text-xs">
                        </div>
                        <div>
                            <label class="text-xs font-semibold block mb-1">Password</label>
                            <input type="password" id="wifi-password" placeholder="Enter password" class="w-full p-2 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded text-xs">
                        </div>
                        <button onclick="connectToWiFi()" class="w-full text-white bg-blue-600 hover:bg-blue-700 font-bold rounded-lg text-sm px-4 py-2 flex justify-center items-center gap-2">
                            <i data-lucide="wifi" class="w-4 h-4"></i> Connect
                        </button>
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center border-b pb-2">
                            <h3 class="font-bold text-gray-400 text-xs uppercase tracking-wider">Available Networks</h3>
                            <button onclick="scanWiFiNetworks()" class="text-xs text-solar-500 font-bold flex gap-1 items-center" title="Scan for nearby networks">
                                <i data-lucide="search" class="w-3 h-3"></i> Scan
                            </button>
                        </div>
                        <div id="wifi-networks-container" class="space-y-2 max-h-48 overflow-y-auto">
                            <p class="text-xs text-gray-400">Click "Scan" to search for networks...</p>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    async function scanWiFiNetworks() {
        ...
        try {
            ...
            if (!data.networks || data.networks.length === 0) {
                container.innerHTML = '<p class="text-xs text-gray-400">No networks found</p>';
                return;
            }
            ...
            container.innerHTML = '<p class="text-xs text-red-500">Scan failed</p>';
        }
    }
    ...
    async function connectToWiFi() {
        ...
        statusEl.className = 'text-xs text-center text-blue-500';
        ...
        try {
            ...
            if (res.ok) {
                statusEl.textContent = 'Connecting to WiFi... Please wait.';
                statusEl.className = 'text-xs text-center text-blue-500';
